...
if (Test-Path "$projectRoot\requirements.txt") {
    try {
        & "$venvPath\Scripts\pip.exe" install -r "$projectRoot\requirements.txt"
...
# Ask if the user wants to run a Python script
$runScript = Read-Host "Would you like to run a Python script? (yes/no)"
if ($runScript -eq "yes") {
    $scriptPath = Read-Host "Enter the full path of the Python script"
    Run-PythonScript -venvPath $venvPath -scriptPath $scriptPath
} else {
    Write-Host "Skipping running a Python script."
    # Optionally, start a Python shell instead
    Start-PythonShell -venvPath $venvPath
}
